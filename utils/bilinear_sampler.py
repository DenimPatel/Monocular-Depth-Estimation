...
def apply_disparity(input_images, x_offset, wrap_mode='border', tensor_type = 'torch.FloatTensor'):
    ...
    # meshgrid function)
    x = torch.linspace(0, width - 1, width).repeat(height, 1).type(tensor_type).to(torch.device(compute_device))
    y = torch.linspace(0, height - 1, height).repeat(width, 1).transpose(0, 1).type(tensor_type).to(torch.device(compute_device))
    ...
    base = dim1 * torch.arange(num_batch).type(tensor_type).to(torch.device(compute_device))
